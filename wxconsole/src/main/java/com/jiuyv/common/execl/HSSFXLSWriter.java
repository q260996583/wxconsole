...
public final class HSSFXLSWriter {
	...
	public static void outputXLS(XLSSheetTemplate sheetTemplate, OutputStream os)
		...
		Map<String,HSSFCellStyle> styles = DefaultCellStyleFactory.createStyles(work);
		short line = 0;
		// create title
		if ( sheetTemplate.getDrawTitle() ) {
			createTitle(
					line++,
					sheet,
		...
		// create sub title
		if ( sheetTemplate.getDrawSubtitle() ) {
			createSubTitle(work, line++, sheet, sheetTemplate.getOperator(),
					sheetTemplate.getOperTime(), sheetTemplate.getColCount());
		}
		// create head
		if ( sheetTemplate.getDrawHead() ) {
			createHead(
					line++,
					sheet,
	...
	private static <T> void createData(short startRow, HSSFSheet sheet,
			List<CellDataType> propNames, List<T> data, HSSFWorkbook wb, Map<String,HSSFCellStyle> styles) throws Exception {
		if (data == null) {
			return;
		}
		...
		for (T tData : data) {
			HSSFRow row = sheet.createRow(rowNum++);
			createRow(row, propNames, tData, wb, styles);
		}
	}
	...
	private static void createRow(HSSFRow row, List<CellDataType> propNames,
			Object data, HSSFWorkbook wb, Map<String,HSSFCellStyle> styles) throws Exception {
		if (row == null) {
			return;
		}
		int colNum = 0;
		HSSFCellStyle style = styles.get("bodyStyle");
		for (CellDataType prop : propNames) {
			HSSFCell cell = row.createCell(colNum++);
			try {
				Object value = PropertyUtils.getProperty(data, prop.getPropName()); 
				// Render Dictionary
				if ( prop.getDict() != null ) {
					Object tmpval = prop.getDict().get(value);
					value = tmpval==null ? value:tmpval;
				}
				if ( style != null  ) {
					cell.setCellStyle(style);
				}
				...
				prop.getRender().render(value, cell, param, wb, styles);
			} catch (Exception e) {
