 ...
 * @version 0.1
 * @author cowyk
 * <h2>AbstractConvertInterceptor.</h2>
 * <p>
 * This class convert the attribute in request scope and store the result after
 * convertion into request.<br />
 * It needs at least 2 parameter which called 'sourceAttrName' and 'converter'
 * to complete the process of convert. 
 * It also provide the target attribute name .<br />
...
public class AbstractConvertInterceptor extends AbstractInterceptor {
	...
	/** The Constant LOGGER. */
	private static final Logger LOGGER = LoggerFactory.getLogger(AbstractConvertInterceptor.class);
	...
	public String intercept(ActionInvocation invocation) 
		...
		invocation.addPreResultListener(new PreResultListener() {
			public void beforeResult(ActionInvocation invocation, 
				...
				if (LOGGER.isDebugEnabled()) {
					LOGGER.debug(
							"beforeResult(ActionInvocation, String) - start");
				}
...
//				request.setAttribute("accessIsSuccessFlag", (String)tarObj);

				if (LOGGER.isDebugEnabled()) {
					LOGGER.debug(
							"beforeResult(ActionInvocation, String) - end");
				}
			}
		});
		//return LOGIN;
...
//		ServletActionContext.getRequest().setAttribute("accessIsSuccessFlag", a);
	...
	protected boolean initInterceptor() {
		...
		// 物件名称都指定时 返回true
		if (sourceAttrName != null && targetAttrName != null) {
			return true;
		}
