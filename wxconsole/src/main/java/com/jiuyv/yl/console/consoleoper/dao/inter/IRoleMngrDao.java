/*
 * Created by KY
 */
package com.jiuyv.yl.console.consoleoper.dao.inter;

import java.util.List;
import java.util.Map;

import com.jiuyv.yl.console.consoleoper.entity.RoleVo;
import com.jiuyv.yl.console.security.entity.RoleResVo;
import com.jiuyv.common.ibatis.vo.ExtData;
import com.jiuyv.common.ibatis.vo.Filter;
import com.jiuyv.common.ibatis.vo.Page;

// TODO: Auto-generated Javadoc
/**
 * The Interface IConsoleOperDao.
 */
public interface IRoleMngrDao {

	
	
	public ExtData<Object> checkUserEdit(Map params) throws Exception;
	public ExtData<RoleVo> findUserRole(List<Filter> filters, Page page) throws Exception;

	public ExtData<RoleVo> doInsertRole(RoleVo roleVo,List<Filter> Filters) throws Exception;
	public List<RoleResVo> findRecListByRole(RoleVo roleVo) throws Exception;
	public void doDeleteRecByRole(RoleVo roleVo) throws Exception;
	public void doInsertRecListByRole(final List<RoleResVo> list) throws Exception;
	  /**
     * 删除角色.
     * 
     * @param roleVo the role vo
     * 
     * @return the ext data< role vo>
     * 
     * @throws Exception the exception
     */
    public ExtData<RoleVo> doDeleteRole(RoleVo roleVo,List<Filter> Filters) throws Exception;
    /**
     * 修改角色.
     * 
     * @param roleVo the role vo
     * 
     * @return the ext data< role vo>
     * 
     * @throws Exception the exception
     */
    public ExtData<RoleVo> doUpdateRole(RoleVo roleVo,List<Filter> Filters) throws Exception;
    

	
	
	
	
	
	

}
