	...
	$.addFlex = function(t, p) {
		...
		var g = {
			...
			dragEnd: function () {
				...
				$('body').noSelect(false);
				var $empty = g.bDiv.empty;
				$empty.css('width',g.hDiv.hbox.css('width'));
			},
			scroll: function () {
			...
			for (i = 0; i < p.buttons.length; i++) {
				...
				if(btn.show){
					...
					for (var si=0;si<showArr.length;si++) {
						...
						count += $.inArray(showArr[si], parent.buttonAutors);
					}
					if(count < 0){
						continue;
					}
		...
		t.grid = g;
		// 添加空行
		$empty = $('<div class="iflexgrid-empty"></div>');
		$empty.css('height','1px');
		$empty.css('width',hbox.css('width'));
		g.bDiv.append($empty);
		g.bDiv.empty = $empty;
		return t;
	};
...
//						if (value !== undefined)
//							$pdiv.append($('<input />').attr('name',property).val(value));
//					}
//				}
				frm.append($pdiv);
				frm.attr('action',url) ;
