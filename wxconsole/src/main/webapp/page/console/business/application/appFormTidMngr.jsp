...
<form id="queryForm" name="queryForm" action="#" method="post"
	...
	<input type="hidden" name="formGroup"  qtype="string" value="01" comparison="eq"/>
...
$(document).ready(function() {
	$('#q_formCode').ajaxSelect({url:'findAppFormDefByFormGroup.action?formGroup=01',valueField:'formCode',displayField:'formName',blank:true});
	...
	var audit= function(record, id) {
		...
		$.ajax({url:'findAppFormTidDetailsById.action',data:{jsonObject:$.toJSON(record)}
			,dataType : 'json'
			,success:function(data) {
				if (data.success) {
	...
	$('#table').flexigrid({ 
		url : 'findAppFormTid.action',
		buttons : [
				{
					name : "审核",
					bclass : 'page_excel',
					show:'ROLE_APP_TID_INFO_AUD',
					id : 'cel',
					onpress : audit
				}],
