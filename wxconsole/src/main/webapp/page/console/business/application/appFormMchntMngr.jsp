...
<form id="queryForm" name="queryForm" action="#" method="post"
	...
	<input type="hidden" name="formGroup"  qtype="string" value="00" comparison="eq"/>
...
$(document).ready(function() {
	$('#q_formCode').ajaxSelect({url:'findAppFormDefByFormGroup.action?formGroup=00',valueField:'formCode',displayField:'formName',blank:true});
	...
	$('#auditDialog').dialog("option","buttons",
			{'通过' : function() {
				if ($.formValidator.pageIsValid('1')) {
					$.ajaxForm({
						formId : 'form1',
						url : "doCheckAppFormMchnt.action",
			...
			'驳回' : function() {
				if ($.formValidator.pageIsValid('1')) {
					$.ajaxForm({
						formId : 'form1',
						url : "doRejectAppFormMchnt.action",
						success : function(data) {
							if (data.success) {
								$('#table').flexRemoveData(data.root[0]);
								$('#auditDialog').dialog('close');
	...
	var audit= function(record, id) {
		...
		$('#include_refNo').val(record.mainCode);
		var formCode = record.formCode;
		$('#form1').fillFormObject({obj:record});
		$.ajax({url:'findAppFormMchntDetailsById.action',data:{jsonObject:$.toJSON(record)}
			,dataType : 'json'
			,success:function(data) {
				if (data.success) {
	...
	$('#table').flexigrid({ 
		url : 'findAppFormMchnt.action',
		buttons : [
				{
					name : "审核",
					bclass : 'page_excel',
					show:'ROLE_APP_MHT_MODI_AUD',
					id : 'cel',
					onpress : audit
				}],
