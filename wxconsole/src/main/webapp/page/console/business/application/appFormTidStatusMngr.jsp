...
<form id="queryForm" name="queryForm" action="#" method="post"
	...
	<input type="hidden" name="formGroup"  qtype="string" value="02" comparison="eq"/>
...
$(document).ready(function() {
	$('#q_formCode').ajaxSelect({url:'findAppFormDefByFormGroup.action?formGroup=02',valueField:'formCode',displayField:'formName',blank:true});
	...
	$('#auditDialog').dialog("option","buttons",
			{'通过' : function() {
				if ($.formValidator.pageIsValid('1')) {
					$.ajaxForm({
						formId : 'form1',
						url : "doCheckAppFormTidStatus.action",
			...
			'驳回' : function() {
				if ($.formValidator.pageIsValid('1')) {
					$.ajaxForm({
						formId : 'form1',
						url : "doRejectAppFormTidStatus.action",
						success : function(data) {
							if (data.success) {
								$('#table').flexRemoveData(data.root[0]);
								$('#auditDialog').dialog('close');
	...
	var audit= function(record, id) {
		...
		$.ajax({url:'findAppFormTidStatusDetailsById.action',data:{jsonObject:$.toJSON(record)}
			,dataType : 'json'
			,success:function(data) {
				if (data.success) {
	...
	$('#table').flexigrid({ 
		url : 'findAppFormTidStatus.action',
		buttons : [
				{
					name : "审核",
					bclass : 'page_excel',
					show:'ROLE_APP_TID_STATUS_AUD',
					id : 'cel',
					onpress : audit
				}],
