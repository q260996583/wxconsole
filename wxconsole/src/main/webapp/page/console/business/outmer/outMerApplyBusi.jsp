   		...
   		<table class="ftable"  cellpadding="1" cellspacing="1" bgcolor="#8392a5" width="100%">
   		  ...
   		  <tr>
   			...
   			<tr>
  				...
  				<td class="fcontent"><input class="qFormInput" disabled id="mccGrpImp" name="mccGrpImpDesc" type="text" /></td>
  			</tr>
  			<tr>
			...
			<tr>
  			...
  			<tr>
   			...
   			<tr>
    			...
    			<td class="fcontent">  <textarea class="qFormInput" id="admitMemo" name="admitMemo" type="text" maxlength="300" ></textarea></td>
   			</tr>
...
$('#outMerDetail').dialog({	autoOpen: false,	bgiframe: true,	modal: true,	resizable: false,	draggable: false,	width: 800,	height: 500
	,title: '详细信息' , buttons: {
		'通过': function(){										
			if ($.formValidator.pageIsValid('1')) {
				...
				$.ajaxForm({formId : 'form2', url : "doApproveOutMerApplyBusi.action",
					success : function(data) {
						if (data.success) {
	...
	,'驳回': function(){										
			if ($.formValidator.pageIsValid('1')) {
				...
				if (!cf) { return; }
				$.ajaxForm({formId : 'form2', url : "doRejectOutMerApplyBusi.action",
					success : function(data) {
						if (data.success) {
							$('#outMerDetail').dialog('close');
							$('#table').refresh();
...
, { display: '安装地址', width: 150, dataIndex: 'tidAddress', align: 'center' }
...
$('#table').flexigrid({url : 'findOutMerApplyBusi.action', width : 400, dir : "desc", sort : "orgCode", usepager : true,	
buttons: [
