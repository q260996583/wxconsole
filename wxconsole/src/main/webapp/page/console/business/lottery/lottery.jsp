...
var getUUID = function() {
	$.ajax({
		...
		url : 'getLoginInfo.action?lotteryType=10',
...
var loop = function () {
	if(true == startFlag) {
		$.ajax({
			type : 'POST',
			url : 'executeRobot.action?uuid=' + uuid + "&lotteryType=10",
...
$(document).ready(function() {
	...
	$('#runRobot').click(function(){
		if(status == 0) {
			$('#qrcode').attr('src','runRobot.action?lotteryType=10&uuid=' + uuid);
			startFlag = true;
		} else {
			alert("机器人已启动!");
		}
	...
	$('#close').click(function(){
		$('#qrcode').attr('src','destoryRobot.action?lotteryType=10');
	});	
