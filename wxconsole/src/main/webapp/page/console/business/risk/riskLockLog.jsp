...
<div id="queryCondition">
	<form id="queryForm" name="queryForm" action="#" method="post">
		<ul class="qry-ul">
			...
			<li><label>延迟结算标识</label><select id="q_lockStatus" name="lockStatus" qtype="string" comparison="eq" multiple="multiple" ctype="REC_LOCK_FLAG" ></select></li>
		</ul>
		<ul class="qry-ul">
			<li><label>延迟结算时间从</label><input id="q_bgnSday" name="oprTime" type="text" xtype="time" qtype="string" comparison="ge" /></li>
			<li><label>到</label><input id="q_endSday" name="oprTime" type="text" xtype="time" qtype="string" comparison="le" /></li>
			<li><label>解除延迟时间从</label><input id="q_appBgnTime" name="unLockTime" type="text" xtype="time" qtype="string" comparison="ge" /></li>
			<li><label>到</label><input id="q_appEndTime" name="unLockTime" type="text" xtype="time" qtype="string" comparison="le" /></li>
			<li><input class='bt' id="queryBtn" value="查询" type="button"/></li>
			<li><input value="重置" class='bt' id="resetBtn" type="button" /></li>
		</ul>
	</form>
</div>
<!-- 列表 -->
...
<div id="viewtabs" style="border: none;">
	...
	<table class="ftable" cellpadding="1" cellspacing="1" bgcolor="#8392a5" width="100%">
		...
		<tr>
			...
			<td class="fcontent"><input class="cdnInput" readonly id="v_mid" name="merName" maxLength="100" type="text" /></td>
		</tr>
		<tr>
		...
		<tr>
			<td class="flabel">延迟清算时间</td>
			<td class="fcontent"><input class="cdnInput" readonly id="v_oprTime" xtype="datetime" name="oprTime" maxLength="20" type="text" /></td>
		</tr>
		<tr>
			<td class="flabel">解除延迟清算时间</td>
			<td class="fcontent"><input class="cdnInput" readonly id="v_unlockTime" xtype="datetime" name="unlockTime" maxLength="20" type="text" /></td>
		</tr>
		<tr>
...
var urls = {
	list:'riskLockLogPage.action',
	excel:'riskLockLogPageExcel.action',
	listRel:'riskLockLogRelPage.action',
	excelRel:'riskLockLogRelPageExcel.action'
};
...
$(function() {
	...
	$('#table').flexigrid({
		...
		colModel : [ 
			...
			,{ display : '延迟清算标识', dataIndex : 'lockStatusDesc', width : 80, align : 'center', sortable:true }
			,{ display : '状态', dataIndex : 'statusDesc', width : 80, align : 'center', sortable:true }
			,{ display : '延迟清算时间', dataIndex : 'oprTime', width : 180, align : 'center', sortable:true, render: fmtTime }
			,{ display : '解除延迟清算时间', dataIndex : 'unlockTime', width : 180, align : 'center', sortable:true, render: fmtTime }			
	...
	$('#queryBtn').click();
	$('#resetBtn').click(function(){$('#queryForm')[0].reset();});
});
//-->
</script>
</body>