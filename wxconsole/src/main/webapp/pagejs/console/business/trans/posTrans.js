...
var  viewPosTransDetail = function(record,id) {
	...
	/*if ( CODE_MAP && record.respCodeLocal ) {
		console.log(record.respCodeLocal);
		var disp = CODE_MAP[record.respCodeLocal];
		if ( disp ) {
			$('#respCodeLocal').val(disp + '['+record.respCodeLocal+']');
		}
	}*/
	...
	$.ajax({
		...
		success:function(data){
			if (data.success) {
				...
				/*if ( CODE_MAP && k.respCodeLocal ) {
					console.log(k.respCodeLocal);
					var disp = CODE_MAP[k.respCodeLocal];
					if ( disp ) {
						$('#respCodeLocal').val(disp + '['+k.respCodeLocal+']');
					}
				}*/
...
$('#table').flexigrid({ 
	url : 'findPosTrans.action', width : 400, dir : "desc", sort : "transSeq", usepager : true, 
	buttons : [
		 {name: "详细信息", bclass: 'page_excel', id: 'cel', onpress: viewPosTransDetail },{separator: true }
	...
	,{ display: '商户结算类型', dataIndex: 'merStatementsType', align: 'center' }
	...
	,{ display: '结算类型', dataIndex: 'stlmAcctSettleTypeDesc', align: 'center' }
	,{ display: '退货类型', dataIndex: 'backTypeDesc', align: 'center' }
	,{ display: '支付机构', dataIndex: 'bankName', align: 'center' }
	]
	});
	...
	$('#contactPosTransDetail').flexigrid({
	    url : 'contactPosTrans.action', width : 800, checkbox : false, usepager : false,					
		colModel : [ 
		 { display: '中心交易流水', dataIndex: 'transSeq', align: 'center', sortable:true }
		,{ display: '终端交易流水', dataIndex: 'tidSeq', align: 'center' }
		,{ display: '本系统清算日', dataIndex: 'pospStmt', sortable:true, render: fmtDate }
		...
		,{ display: '冲正标志', dataIndex: 'rollbackFlagZh', align: 'center' }
		,{ display: '撤销标志', dataIndex: 'cancelFlagZh', align: 'center' }
		,{ display: '清算标志', dataIndex: 'settleFlagZh', align: 'center' }
		,{ display: '退款类型', dataIndex: 'signsZh', align: 'center' }
		,{ display: '退款数额', dataIndex: 'signsZh', align: 'center' }
		,{ display: '已退款金额', dataIndex: 'backAmtDesc', align: 'center' }
		]
	});	
