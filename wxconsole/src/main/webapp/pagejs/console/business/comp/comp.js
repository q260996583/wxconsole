...
function fun() {            
	 //查询
    $('#table2').flexigrid_comp({
    	type 	: 'POST',
        url : 'doFindCompDetail.action',
        width : 400,
        sort : "compCode",
        dir : 'desc',
        checkbox : false,
        ...
        colModel : [ {
            ...
            display : '服务状态',
            dataIndex : 'serviceStatue',
            align : 'center',
            decode:{"0":"拒绝服务中","1":"服务中"},
    		sortable:true
        }, {
            display : '网络连接状态',
            dataIndex : 'netWorkFlag',
            align : 'center',
            decode:{"0":"网络或联网业务失败","1":"连接"},
    		sortable:true
        }, {
            display : '最近更新时间',
            dataIndex : 'oprTime',
    ...
    //查询
    $('#table1').flexigrid_comp({
    	type 	: 'POST',
        url : 'doFindCompNumber.action',
        width : 400,
        sort : "compCode",
        dir : 'desc',
        checkbox : false,
...
//         tableId : 'table2'
//    });
    $('#table1').flexReload("a","a");
    $('#table2').flexReload("a","a");
				...
				$.each(p.viewStore, function(i, row) {
					...
					$("#table1").find("tr:gt(0)").each(function() { 
						...
						$(this).find("td").each(function(i,n) {
							number[i] = n.innerHTML;	
						});	
						var gatewayNum = number[1];
						var normalNum = number[2];
						if(gatewayNum == normalNum){
							$(this).css("background-color","greed") ;
						}else if(normalNum == 0){
