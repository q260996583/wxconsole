	    ...
	    /*$.ajax({type:"post", url:"checkBlackMerChant.action", dataType:"json",
	        ...
	        success:function(data) {
	            if (!data.success) { alert(data.syserr); return false;
	                ...
	                $.ajax({type:"post", url:"doUpdateQrcMerchant.action", dataType:"json", data:{ mers: $.toJSON(mhtObj) },
	                    success:function(data) {
	                        if (data.success) {
...
var updMerDeleteBtn = function() {
	...
	alert("hahaha");
	$.ajax({url: 'delQrcMerAdd.action', type: 'POST', dataType:'json', data:{instMerId: instMerId,instId:instId}
		,success : function(data) {
			if (data.success) {
				alert('操作成功');
				$(this).dialog('close');
...
//	$('#form3 :input').attr('disabled', false);
	...
	$('#editMer').dialog('option','buttons',{'更新': updMerSaveBtn, '关闭':function(){ $(this).dialog('close'); }});
	$('#editMer').dialog('open');
...
$('#table').flexigrid({url: 'findOutQrcMer.action', width: 400, sort: "instMerId", dir: "desc", usepager: true, checkbox: false,
buttons : [
