	...
	$('#addtid').dialog("option","buttons",
		{'更新' : function() {
			if ($.formValidator.pageIsValid('1')) {
				...
				$.ajaxForm({
					formId : 'form1',
					url : "doUpdateConsoleNoticeInfo.action",
					dealData:function(data){																								
						data.userStr=selectRes; return data;
					},
					success : function(data) {
						if (data.success) {
...
var newtid=function() {
	$('#formreset').click();
	var urlStr= 'findConsoleNoticeUserTree.action';
	$('#resourceTree').jsTree({url:urlStr,theme_name:'checkbox'});		
	...
	$('#addtid').dialog("option","buttons",{
		'确定' : function() {
			if ($.formValidator.pageIsValid('1')) {
				...
				$.ajaxForm({formId : 'form1', url : "doInsertConsoleNoticeInfo.action",
					dealData:function(data){data.userStr=selectRes; return data; },
					success : function(data) {
						if (data.success) {
							$('#addtid').dialog('close');
...
var commit = function(record,id){
	...
	if(able){
		$.ajax({url:'doCommitConsoleNoticeInfo.action',
...
var deleteNotice = function(record,id){
	...
	if(able){
		$.ajax({url:'doDeleteConsoleNoticeInfo.action',
			data:{jsonObject:$.toJSON(record)},dataType:'json',
			success:function(data){
				if(data.success){
					alert('删除成功');
...
$('#table').flexigrid({ 
url : 'findConsoleNoticeInfoSelf.action',dir : "desc", sort : "noticeSeq", usepager : true, dbClickRecord: viewtid ,
buttons : [
 { name : "新增", bclass : 'grid_add', id : 'txt', show: 'ROLE_CONSOLE_NOTICE_INFO_ADD', onpress :newtid },{ separator : true }
,{ name : "修改", bclass : 'grid_edit', id : 'txt', show: 'ROLE_CONSOLE_NOTICE_INFO_UPD', onpress : edittid },{ separator : true }
,{ name : "提交", bclass : 'grid_edit', id : 'txt', show: 'ROLE_CONSOLE_NOTICE_INFO_COMMIT', onpress : commit },{ separator : true }
,{ name : "删除", bclass : 'grid_del', id : 'txt', show: 'ROLE_CONSOLE_NOTICE_INFO_DEL', onpress : deleteNotice },{ separator : true }
...
bindPreview({btn:'v_previewBtn', compId:'v_content', fldId:'content', preview:'findNoticePreview.action'});
bindKindEditor({btnId:'a_kindEditBtn',textareaId:'content',uploadUrl:'uploadImgInKindEditorInnerNotice.action'});
